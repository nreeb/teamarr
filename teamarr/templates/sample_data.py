"""Sample data for template variable previews.

Sport-specific sample values for the template variable picker.
Used for live preview in the UI.
"""

# Available sports for the dropdown
AVAILABLE_SPORTS = ["NBA", "NFL", "MLB", "NHL", "NCAAM", "NCAAF", "Soccer"]

# Sample data organized by variable name and sport
# Each variable can have different sample values per sport
SAMPLE_DATA: dict[str, dict[str, str]] = {
    # Team Identity
    "team_name": {
        "NBA": "Detroit Pistons",
        "NFL": "Detroit Lions",
        "MLB": "Detroit Tigers",
        "NHL": "Detroit Red Wings",
        "NCAAM": "Michigan Wolverines",
        "NCAAF": "Michigan Wolverines",
        "Soccer": "Liverpool",
    },
    "team_abbrev": {
        "NBA": "DET",
        "NFL": "DET",
        "MLB": "DET",
        "NHL": "DET",
        "NCAAM": "MICH",
        "NCAAF": "MICH",
        "Soccer": "LIV",
    },
    "league": {
        "NBA": "NBA",
        "NFL": "NFL",
        "MLB": "MLB",
        "NHL": "NHL",
        "NCAAM": "NCAAM",
        "NCAAF": "NCAAF",
        "Soccer": "Premier League",
    },
    "sport": {
        "NBA": "Basketball",
        "NFL": "Football",
        "MLB": "Baseball",
        "NHL": "Hockey",
        "NCAAM": "Basketball",
        "NCAAF": "Football",
        "Soccer": "Soccer",
    },
    # Opponent/Matchup
    "opponent": {
        "NBA": "Chicago Bulls",
        "NFL": "Chicago Bears",
        "MLB": "Chicago White Sox",
        "NHL": "Chicago Blackhawks",
        "NCAAM": "Ohio State Buckeyes",
        "NCAAF": "Ohio State Buckeyes",
        "Soccer": "Manchester City",
    },
    "opponent.next": {
        "NBA": "Milwaukee Bucks",
        "NFL": "Green Bay Packers",
        "MLB": "Cleveland Guardians",
        "NHL": "Columbus Blue Jackets",
        "NCAAM": "Indiana Hoosiers",
        "NCAAF": "Penn State Nittany Lions",
        "Soccer": "Arsenal",
    },
    "opponent.last": {
        "NBA": "Cleveland Cavaliers",
        "NFL": "Minnesota Vikings",
        "MLB": "Minnesota Twins",
        "NHL": "Minnesota Wild",
        "NCAAM": "Michigan State Spartans",
        "NCAAF": "Michigan State Spartans",
        "Soccer": "Chelsea",
    },
    "matchup": {
        "NBA": "Detroit Pistons vs Chicago Bulls",
        "NFL": "Detroit Lions vs Chicago Bears",
        "MLB": "Detroit Tigers vs Chicago White Sox",
        "NHL": "Detroit Red Wings vs Chicago Blackhawks",
        "NCAAM": "Michigan Wolverines vs Ohio State Buckeyes",
        "NCAAF": "Michigan Wolverines vs Ohio State Buckeyes",
        "Soccer": "Liverpool vs Manchester City",
    },
    "home_team": {
        "NBA": "Detroit Pistons",
        "NFL": "Detroit Lions",
        "MLB": "Detroit Tigers",
        "NHL": "Detroit Red Wings",
        "NCAAM": "Michigan Wolverines",
        "NCAAF": "Michigan Wolverines",
        "Soccer": "Liverpool",
    },
    "away_team": {
        "NBA": "Chicago Bulls",
        "NFL": "Chicago Bears",
        "MLB": "Chicago White Sox",
        "NHL": "Chicago Blackhawks",
        "NCAAM": "Ohio State Buckeyes",
        "NCAAF": "Ohio State Buckeyes",
        "Soccer": "Manchester City",
    },
    "home_team.next": {
        "NBA": "Milwaukee Bucks",
        "NFL": "Green Bay Packers",
        "MLB": "Cleveland Guardians",
        "NHL": "Columbus Blue Jackets",
        "NCAAM": "Indiana Hoosiers",
        "NCAAF": "Penn State Nittany Lions",
        "Soccer": "Arsenal",
    },
    "away_team.next": {
        "NBA": "Detroit Pistons",
        "NFL": "Detroit Lions",
        "MLB": "Detroit Tigers",
        "NHL": "Detroit Red Wings",
        "NCAAM": "Michigan Wolverines",
        "NCAAF": "Michigan Wolverines",
        "Soccer": "Liverpool",
    },
    "home_team.last": {
        "NBA": "Cleveland Cavaliers",
        "NFL": "Minnesota Vikings",
        "MLB": "Minnesota Twins",
        "NHL": "Minnesota Wild",
        "NCAAM": "Michigan State Spartans",
        "NCAAF": "Michigan State Spartans",
        "Soccer": "Chelsea",
    },
    "away_team.last": {
        "NBA": "Detroit Pistons",
        "NFL": "Detroit Lions",
        "MLB": "Detroit Tigers",
        "NHL": "Detroit Red Wings",
        "NCAAM": "Michigan Wolverines",
        "NCAAF": "Michigan Wolverines",
        "Soccer": "Liverpool",
    },
    # Records
    "team_record": {
        "NBA": "25-15",
        "NFL": "10-4",
        "MLB": "82-65",
        "NHL": "28-18-6",
        "NCAAM": "18-5",
        "NCAAF": "11-1",
        "Soccer": "15-3-2",
    },
    "opponent_record": {
        "NBA": "22-18",
        "NFL": "8-6",
        "MLB": "75-72",
        "NHL": "25-20-7",
        "NCAAM": "16-7",
        "NCAAF": "10-2",
        "Soccer": "14-4-2",
    },
    "home_team_record": {
        "NBA": "25-15",
        "NFL": "10-4",
        "MLB": "82-65",
        "NHL": "28-18-6",
        "NCAAM": "18-5",
        "NCAAF": "11-1",
        "Soccer": "15-3-2",
    },
    "away_team_record": {
        "NBA": "22-18",
        "NFL": "8-6",
        "MLB": "75-72",
        "NHL": "25-20-7",
        "NCAAM": "16-7",
        "NCAAF": "10-2",
        "Soccer": "14-4-2",
    },
    "home_team_record.next": {
        "NBA": "26-15",
        "NFL": "11-4",
        "MLB": "83-65",
        "NHL": "29-18-6",
        "NCAAM": "19-5",
        "NCAAF": "12-1",
        "Soccer": "16-3-2",
    },
    "away_team_record.next": {
        "NBA": "20-20",
        "NFL": "9-6",
        "MLB": "70-77",
        "NHL": "22-22-8",
        "NCAAM": "14-9",
        "NCAAF": "8-4",
        "Soccer": "12-5-3",
    },
    # Venue
    "venue": {
        "NBA": "Little Caesars Arena",
        "NFL": "Ford Field",
        "MLB": "Comerica Park",
        "NHL": "Little Caesars Arena",
        "NCAAM": "Crisler Center",
        "NCAAF": "Michigan Stadium",
        "Soccer": "Anfield",
    },
    "venue_city": {
        "NBA": "Detroit",
        "NFL": "Detroit",
        "MLB": "Detroit",
        "NHL": "Detroit",
        "NCAAM": "Ann Arbor",
        "NCAAF": "Ann Arbor",
        "Soccer": "Liverpool",
    },
    "venue_state": {
        "NBA": "MI",
        "NFL": "MI",
        "MLB": "MI",
        "NHL": "MI",
        "NCAAM": "MI",
        "NCAAF": "MI",
        "Soccer": "England",
    },
    "venue_full": {
        "NBA": "Little Caesars Arena, Detroit, MI",
        "NFL": "Ford Field, Detroit, MI",
        "MLB": "Comerica Park, Detroit, MI",
        "NHL": "Little Caesars Arena, Detroit, MI",
        "NCAAM": "Crisler Center, Ann Arbor, MI",
        "NCAAF": "Michigan Stadium, Ann Arbor, MI",
        "Soccer": "Anfield, Liverpool, England",
    },
    "venue.next": {
        "NBA": "Fiserv Forum",
        "NFL": "Lambeau Field",
        "MLB": "Progressive Field",
        "NHL": "Nationwide Arena",
        "NCAAM": "Simon Skjodt Assembly Hall",
        "NCAAF": "Beaver Stadium",
        "Soccer": "Emirates Stadium",
    },
    "venue_city.next": {
        "NBA": "Milwaukee",
        "NFL": "Green Bay",
        "MLB": "Cleveland",
        "NHL": "Columbus",
        "NCAAM": "Bloomington",
        "NCAAF": "University Park",
        "Soccer": "London",
    },
    "venue_state.next": {
        "NBA": "WI",
        "NFL": "WI",
        "MLB": "OH",
        "NHL": "OH",
        "NCAAM": "IN",
        "NCAAF": "PA",
        "Soccer": "England",
    },
    "venue_full.next": {
        "NBA": "Fiserv Forum, Milwaukee, WI",
        "NFL": "Lambeau Field, Green Bay, WI",
        "MLB": "Progressive Field, Cleveland, OH",
        "NHL": "Nationwide Arena, Columbus, OH",
        "NCAAM": "Simon Skjodt Assembly Hall, Bloomington, IN",
        "NCAAF": "Beaver Stadium, University Park, PA",
        "Soccer": "Emirates Stadium, London, England",
    },
    # Date/Time
    "game_date": {
        "NBA": "Sunday, Dec 22",
        "NFL": "Sunday, Dec 22",
        "MLB": "Saturday, Jul 12",
        "NHL": "Saturday, Dec 21",
        "NCAAM": "Saturday, Jan 11",
        "NCAAF": "Saturday, Nov 30",
        "Soccer": "Saturday, Dec 21",
    },
    "game_date.next": {
        "NBA": "Tuesday, Dec 24",
        "NFL": "Sunday, Dec 29",
        "MLB": "Sunday, Jul 13",
        "NHL": "Monday, Dec 23",
        "NCAAM": "Tuesday, Jan 14",
        "NCAAF": "Saturday, Dec 7",
        "Soccer": "Wednesday, Dec 25",
    },
    "game_date.last": {
        "NBA": "Friday, Dec 20",
        "NFL": "Sunday, Dec 15",
        "MLB": "Friday, Jul 11",
        "NHL": "Thursday, Dec 19",
        "NCAAM": "Wednesday, Jan 8",
        "NCAAF": "Saturday, Nov 23",
        "Soccer": "Sunday, Dec 15",
    },
    "game_time": {
        "NBA": "7:00 PM EST",
        "NFL": "1:00 PM EST",
        "MLB": "4:10 PM EDT",
        "NHL": "7:00 PM EST",
        "NCAAM": "2:00 PM EST",
        "NCAAF": "12:00 PM EST",
        "Soccer": "10:00 AM EST",
    },
    "game_time.next": {
        "NBA": "8:00 PM EST",
        "NFL": "4:25 PM EST",
        "MLB": "1:10 PM EDT",
        "NHL": "7:30 PM EST",
        "NCAAM": "7:00 PM EST",
        "NCAAF": "3:30 PM EST",
        "Soccer": "2:30 PM EST",
    },
    # Today/Tonight
    "today_tonight": {
        "NBA": "tonight",
        "NFL": "today",
        "MLB": "tonight",
        "NHL": "tonight",
        "NCAAM": "today",
        "NCAAF": "today",
        "Soccer": "today",
    },
    "today_tonight.next": {
        "NBA": "tonight",
        "NFL": "today",
        "MLB": "tonight",
        "NHL": "tonight",
        "NCAAM": "tonight",
        "NCAAF": "today",
        "Soccer": "today",
    },
    "today_tonight_title": {
        "NBA": "Tonight",
        "NFL": "Today",
        "MLB": "Tonight",
        "NHL": "Tonight",
        "NCAAM": "Today",
        "NCAAF": "Today",
        "Soccer": "Today",
    },
    "today_tonight_title.next": {
        "NBA": "Tonight",
        "NFL": "Today",
        "MLB": "Tonight",
        "NHL": "Tonight",
        "NCAAM": "Tonight",
        "NCAAF": "Today",
        "Soccer": "Today",
    },
    # Home/Away (vs_at uses "at" text, vs_@ uses "@" symbol)
    "vs_at": {
        "NBA": "vs",
        "NFL": "vs",
        "MLB": "vs",
        "NHL": "vs",
        "NCAAM": "vs",
        "NCAAF": "vs",
        "Soccer": "vs",
    },
    "vs_at.next": {
        "NBA": "at",
        "NFL": "vs",
        "MLB": "at",
        "NHL": "at",
        "NCAAM": "at",
        "NCAAF": "vs",
        "Soccer": "at",
    },
    "is_home": {
        "NBA": "true",
        "NFL": "true",
        "MLB": "true",
        "NHL": "true",
        "NCAAM": "true",
        "NCAAF": "true",
        "Soccer": "true",
    },
    # Scores/Results (only .last)
    "final_score": {
        "NBA": "112-105",
        "NFL": "31-24",
        "MLB": "5-3",
        "NHL": "4-2",
        "NCAAM": "78-65",
        "NCAAF": "42-27",
        "Soccer": "2-1",
    },
    "final_score.last": {
        "NBA": "108-102",
        "NFL": "28-21",
        "MLB": "7-4",
        "NHL": "3-2",
        "NCAAM": "85-72",
        "NCAAF": "35-28",
        "Soccer": "3-0",
    },
    "result_text": {
        "NBA": "defeated",
        "NFL": "defeated",
        "MLB": "defeated",
        "NHL": "defeated",
        "NCAAM": "defeated",
        "NCAAF": "defeated",
        "Soccer": "defeated",
    },
    "result_text.last": {
        "NBA": "defeated",
        "NFL": "beat",
        "MLB": "beat",
        "NHL": "beat",
        "NCAAM": "defeated",
        "NCAAF": "defeated",
        "Soccer": "beat",
    },
    # Broadcast
    "broadcast_network": {
        "NBA": "ESPN, Bally Sports Detroit",
        "NFL": "FOX, NFL Network",
        "MLB": "Bally Sports Detroit",
        "NHL": "ESPN+, Bally Sports Detroit",
        "NCAAM": "CBS, ESPN+",
        "NCAAF": "ABC, ESPN",
        "Soccer": "NBC, Peacock",
    },
    "broadcast_simple": {
        "NBA": "ESPN",
        "NFL": "FOX",
        "MLB": "Bally Sports",
        "NHL": "ESPN+",
        "NCAAM": "CBS",
        "NCAAF": "ABC",
        "Soccer": "NBC",
    },
    # Odds
    "odds_spread": {
        "NBA": "-4.5",
        "NFL": "-3.0",
        "MLB": "-1.5",
        "NHL": "-1.5",
        "NCAAM": "-7.0",
        "NCAAF": "-14.5",
        "Soccer": "-0.5",
    },
    "odds_moneyline": {
        "NBA": "-180",
        "NFL": "-150",
        "MLB": "-130",
        "NHL": "-140",
        "NCAAM": "-275",
        "NCAAF": "-500",
        "Soccer": "-120",
    },
    "odds_over_under": {
        "NBA": "225.5",
        "NFL": "48.5",
        "MLB": "8.5",
        "NHL": "6.5",
        "NCAAM": "145.5",
        "NCAAF": "54.5",
        "Soccer": "2.5",
    },
    # Streaks - Our Team
    "streak": {
        "NBA": "W3",
        "NFL": "W2",
        "MLB": "W4",
        "NHL": "L2",
        "NCAAM": "W5",
        "NCAAF": "W8",
        "Soccer": "W4",
    },
    "streak_length": {
        "NBA": "3",
        "NFL": "2",
        "MLB": "4",
        "NHL": "2",
        "NCAAM": "5",
        "NCAAF": "8",
        "Soccer": "4",
    },
    "streak_type": {
        "NBA": "win",
        "NFL": "win",
        "MLB": "win",
        "NHL": "loss",
        "NCAAM": "win",
        "NCAAF": "win",
        "Soccer": "win",
    },
    "win_streak": {
        "NBA": "3",
        "NFL": "2",
        "MLB": "4",
        "NHL": "",  # On losing streak
        "NCAAM": "5",
        "NCAAF": "8",
        "Soccer": "4",
    },
    "loss_streak": {
        "NBA": "",  # On winning streak
        "NFL": "",
        "MLB": "",
        "NHL": "2",
        "NCAAM": "",
        "NCAAF": "",
        "Soccer": "",
    },
    # Streaks - Opponent
    "opponent_streak": {
        "NBA": "L3",
        "NFL": "L1",
        "MLB": "W2",
        "NHL": "W4",
        "NCAAM": "L2",
        "NCAAF": "W3",
        "Soccer": "L1",
    },
    "opponent_streak_length": {
        "NBA": "3",
        "NFL": "1",
        "MLB": "2",
        "NHL": "4",
        "NCAAM": "2",
        "NCAAF": "3",
        "Soccer": "1",
    },
    "opponent_streak_type": {
        "NBA": "loss",
        "NFL": "loss",
        "MLB": "win",
        "NHL": "win",
        "NCAAM": "loss",
        "NCAAF": "win",
        "Soccer": "loss",
    },
    "opponent_win_streak": {
        "NBA": "",
        "NFL": "",
        "MLB": "2",
        "NHL": "4",
        "NCAAM": "",
        "NCAAF": "3",
        "Soccer": "",
    },
    "opponent_loss_streak": {
        "NBA": "3",
        "NFL": "1",
        "MLB": "",
        "NHL": "",
        "NCAAM": "2",
        "NCAAF": "",
        "Soccer": "1",
    },
    # Streaks - Home/Away Team
    "home_team_streak": {
        "NBA": "W3",
        "NFL": "W2",
        "MLB": "W4",
        "NHL": "L2",
        "NCAAM": "W5",
        "NCAAF": "W8",
        "Soccer": "W4",
    },
    "home_team_streak_length": {
        "NBA": "3",
        "NFL": "2",
        "MLB": "4",
        "NHL": "2",
        "NCAAM": "5",
        "NCAAF": "8",
        "Soccer": "4",
    },
    "home_team_win_streak": {
        "NBA": "3",
        "NFL": "2",
        "MLB": "4",
        "NHL": "",
        "NCAAM": "5",
        "NCAAF": "8",
        "Soccer": "4",
    },
    "home_team_loss_streak": {
        "NBA": "",
        "NFL": "",
        "MLB": "",
        "NHL": "2",
        "NCAAM": "",
        "NCAAF": "",
        "Soccer": "",
    },
    "away_team_streak": {
        "NBA": "L3",
        "NFL": "L1",
        "MLB": "W2",
        "NHL": "W4",
        "NCAAM": "L2",
        "NCAAF": "W3",
        "Soccer": "L1",
    },
    "away_team_streak_length": {
        "NBA": "3",
        "NFL": "1",
        "MLB": "2",
        "NHL": "4",
        "NCAAM": "2",
        "NCAAF": "3",
        "Soccer": "1",
    },
    "away_team_win_streak": {
        "NBA": "",
        "NFL": "",
        "MLB": "2",
        "NHL": "4",
        "NCAAM": "",
        "NCAAF": "3",
        "Soccer": "",
    },
    "away_team_loss_streak": {
        "NBA": "3",
        "NFL": "1",
        "MLB": "",
        "NHL": "",
        "NCAAM": "2",
        "NCAAF": "",
        "Soccer": "1",
    },
    # Opponent streak .next/.last variants
    "opponent_streak.next": {
        "NBA": "W2",
        "NFL": "W3",
        "MLB": "L1",
        "NHL": "W2",
        "NCAAM": "W4",
        "NCAAF": "L2",
        "Soccer": "W3",
    },
    "opponent_streak.last": {
        "NBA": "W1",
        "NFL": "L2",
        "MLB": "W3",
        "NHL": "L1",
        "NCAAM": "W2",
        "NCAAF": "W5",
        "Soccer": "L2",
    },
    "opponent_streak_length.next": {
        "NBA": "2",
        "NFL": "3",
        "MLB": "1",
        "NHL": "2",
        "NCAAM": "4",
        "NCAAF": "2",
        "Soccer": "3",
    },
    "opponent_streak_length.last": {
        "NBA": "1",
        "NFL": "2",
        "MLB": "3",
        "NHL": "1",
        "NCAAM": "2",
        "NCAAF": "5",
        "Soccer": "2",
    },
    "opponent_streak_type.next": {
        "NBA": "win",
        "NFL": "win",
        "MLB": "loss",
        "NHL": "win",
        "NCAAM": "win",
        "NCAAF": "loss",
        "Soccer": "win",
    },
    "opponent_streak_type.last": {
        "NBA": "win",
        "NFL": "loss",
        "MLB": "win",
        "NHL": "loss",
        "NCAAM": "win",
        "NCAAF": "win",
        "Soccer": "loss",
    },
    "opponent_win_streak.next": {
        "NBA": "2",
        "NFL": "3",
        "MLB": "",
        "NHL": "2",
        "NCAAM": "4",
        "NCAAF": "",
        "Soccer": "3",
    },
    "opponent_win_streak.last": {
        "NBA": "1",
        "NFL": "",
        "MLB": "3",
        "NHL": "",
        "NCAAM": "2",
        "NCAAF": "5",
        "Soccer": "",
    },
    "opponent_loss_streak.next": {
        "NBA": "",
        "NFL": "",
        "MLB": "1",
        "NHL": "",
        "NCAAM": "",
        "NCAAF": "2",
        "Soccer": "",
    },
    "opponent_loss_streak.last": {
        "NBA": "",
        "NFL": "2",
        "MLB": "",
        "NHL": "1",
        "NCAAM": "",
        "NCAAF": "",
        "Soccer": "2",
    },
    # Home team streak .next/.last variants
    "home_team_streak.next": {
        "NBA": "W4",
        "NFL": "W3",
        "MLB": "L2",
        "NHL": "W5",
        "NCAAM": "W6",
        "NCAAF": "W9",
        "Soccer": "W5",
    },
    "home_team_streak.last": {
        "NBA": "W2",
        "NFL": "W1",
        "MLB": "W3",
        "NHL": "L1",
        "NCAAM": "W4",
        "NCAAF": "W7",
        "Soccer": "W3",
    },
    "home_team_streak_length.next": {
        "NBA": "4",
        "NFL": "3",
        "MLB": "2",
        "NHL": "5",
        "NCAAM": "6",
        "NCAAF": "9",
        "Soccer": "5",
    },
    "home_team_streak_length.last": {
        "NBA": "2",
        "NFL": "1",
        "MLB": "3",
        "NHL": "1",
        "NCAAM": "4",
        "NCAAF": "7",
        "Soccer": "3",
    },
    "home_team_win_streak.next": {
        "NBA": "4",
        "NFL": "3",
        "MLB": "",
        "NHL": "5",
        "NCAAM": "6",
        "NCAAF": "9",
        "Soccer": "5",
    },
    "home_team_win_streak.last": {
        "NBA": "2",
        "NFL": "1",
        "MLB": "3",
        "NHL": "",
        "NCAAM": "4",
        "NCAAF": "7",
        "Soccer": "3",
    },
    "home_team_loss_streak.next": {
        "NBA": "",
        "NFL": "",
        "MLB": "2",
        "NHL": "",
        "NCAAM": "",
        "NCAAF": "",
        "Soccer": "",
    },
    "home_team_loss_streak.last": {
        "NBA": "",
        "NFL": "",
        "MLB": "",
        "NHL": "1",
        "NCAAM": "",
        "NCAAF": "",
        "Soccer": "",
    },
    # Away team streak .next/.last variants
    "away_team_streak.next": {
        "NBA": "L2",
        "NFL": "W2",
        "MLB": "W1",
        "NHL": "W3",
        "NCAAM": "L1",
        "NCAAF": "W4",
        "Soccer": "L2",
    },
    "away_team_streak.last": {
        "NBA": "L4",
        "NFL": "L2",
        "MLB": "W1",
        "NHL": "W5",
        "NCAAM": "L3",
        "NCAAF": "W2",
        "Soccer": "L1",
    },
    "away_team_streak_length.next": {
        "NBA": "2",
        "NFL": "2",
        "MLB": "1",
        "NHL": "3",
        "NCAAM": "1",
        "NCAAF": "4",
        "Soccer": "2",
    },
    "away_team_streak_length.last": {
        "NBA": "4",
        "NFL": "2",
        "MLB": "1",
        "NHL": "5",
        "NCAAM": "3",
        "NCAAF": "2",
        "Soccer": "1",
    },
    "away_team_win_streak.next": {
        "NBA": "",
        "NFL": "2",
        "MLB": "1",
        "NHL": "3",
        "NCAAM": "",
        "NCAAF": "4",
        "Soccer": "",
    },
    "away_team_win_streak.last": {
        "NBA": "",
        "NFL": "",
        "MLB": "1",
        "NHL": "5",
        "NCAAM": "",
        "NCAAF": "2",
        "Soccer": "",
    },
    "away_team_loss_streak.next": {
        "NBA": "2",
        "NFL": "",
        "MLB": "",
        "NHL": "",
        "NCAAM": "1",
        "NCAAF": "",
        "Soccer": "2",
    },
    "away_team_loss_streak.last": {
        "NBA": "4",
        "NFL": "2",
        "MLB": "",
        "NHL": "",
        "NCAAM": "3",
        "NCAAF": "",
        "Soccer": "1",
    },
    # Rankings (college)
    "team_rank": {
        "NBA": "",
        "NFL": "",
        "MLB": "",
        "NHL": "",
        "NCAAM": "#8",
        "NCAAF": "#3",
        "Soccer": "",
    },
    "team_rank_display": {
        "NBA": "",
        "NFL": "",
        "MLB": "",
        "NHL": "",
        "NCAAM": "(#8)",
        "NCAAF": "(#3)",
        "Soccer": "",
    },
    "opponent_rank": {
        "NBA": "",
        "NFL": "",
        "MLB": "",
        "NHL": "",
        "NCAAM": "#12",
        "NCAAF": "#7",
        "Soccer": "",
    },
    "opponent_rank_display": {
        "NBA": "",
        "NFL": "",
        "MLB": "",
        "NHL": "",
        "NCAAM": "(#12)",
        "NCAAF": "(#7)",
        "Soccer": "",
    },
    # Playoff/Season type
    "season_type": {
        "NBA": "Regular Season",
        "NFL": "Regular Season",
        "MLB": "Regular Season",
        "NHL": "Regular Season",
        "NCAAM": "Regular Season",
        "NCAAF": "Bowl Season",
        "Soccer": "Premier League",
    },
    # === ADDITIONAL CRITICAL VARIABLES ===
    # Gracenote category (for EPG title)
    "gracenote_category": {
        "NBA": "NBA Basketball",
        "NFL": "NFL Football",
        "MLB": "MLB Baseball",
        "NHL": "NHL Hockey",
        "NCAAM": "Men's College Basketball",
        "NCAAF": "College Football",
        "Soccer": "Premier League Soccer",
    },
    # League variants
    "league_name": {
        "NBA": "National Basketball Association",
        "NFL": "National Football League",
        "MLB": "Major League Baseball",
        "NHL": "National Hockey League",
        "NCAAM": "NCAA Men's Basketball",
        "NCAAF": "NCAA Football",
        "Soccer": "English Premier League",
    },
    "league_id": {
        "NBA": "nba",
        "NFL": "nfl",
        "MLB": "mlb",
        "NHL": "nhl",
        "NCAAM": "ncaam",
        "NCAAF": "ncaaf",
        "Soccer": "epl",
    },
    "league_code": {
        "NBA": "nba",
        "NFL": "nfl",
        "MLB": "mlb",
        "NHL": "nhl",
        "NCAAM": "mens-college-basketball",
        "NCAAF": "college-football",
        "Soccer": "eng.1",
    },
    "sport_lower": {
        "NBA": "basketball",
        "NFL": "football",
        "MLB": "baseball",
        "NHL": "hockey",
        "NCAAM": "basketball",
        "NCAAF": "football",
        "Soccer": "soccer",
    },
    # Abbreviation variants
    "team_abbrev_lower": {
        "NBA": "det",
        "NFL": "det",
        "MLB": "det",
        "NHL": "det",
        "NCAAM": "mich",
        "NCAAF": "mich",
        "Soccer": "liv",
    },
    "team_name_pascal": {
        "NBA": "DetroitPistons",
        "NFL": "DetroitLions",
        "MLB": "DetroitTigers",
        "NHL": "DetroitRedWings",
        "NCAAM": "MichiganWolverines",
        "NCAAF": "MichiganWolverines",
        "Soccer": "Liverpool",
    },
    "opponent_abbrev": {
        "NBA": "CHI",
        "NFL": "CHI",
        "MLB": "CWS",
        "NHL": "CHI",
        "NCAAM": "OSU",
        "NCAAF": "OSU",
        "Soccer": "MCI",
    },
    "opponent_abbrev_lower": {
        "NBA": "chi",
        "NFL": "chi",
        "MLB": "cws",
        "NHL": "chi",
        "NCAAM": "osu",
        "NCAAF": "osu",
        "Soccer": "mci",
    },
    "home_team_abbrev": {
        "NBA": "DET",
        "NFL": "DET",
        "MLB": "DET",
        "NHL": "DET",
        "NCAAM": "MICH",
        "NCAAF": "MICH",
        "Soccer": "LIV",
    },
    "away_team_abbrev": {
        "NBA": "CHI",
        "NFL": "CHI",
        "MLB": "CWS",
        "NHL": "CHI",
        "NCAAM": "OSU",
        "NCAAF": "OSU",
        "Soccer": "MCI",
    },
    "matchup_abbrev": {
        "NBA": "DET vs CHI",
        "NFL": "DET vs CHI",
        "MLB": "DET vs CWS",
        "NHL": "DET vs CHI",
        "NCAAM": "MICH vs OSU",
        "NCAAF": "MICH vs OSU",
        "Soccer": "LIV vs MCI",
    },
    # Scores
    "team_score": {
        "NBA": "112",
        "NFL": "31",
        "MLB": "5",
        "NHL": "4",
        "NCAAM": "78",
        "NCAAF": "42",
        "Soccer": "2",
    },
    "opponent_score": {
        "NBA": "105",
        "NFL": "24",
        "MLB": "3",
        "NHL": "2",
        "NCAAM": "65",
        "NCAAF": "27",
        "Soccer": "1",
    },
    "score": {
        "NBA": "112-105",
        "NFL": "31-24",
        "MLB": "5-3",
        "NHL": "4-2",
        "NCAAM": "78-65",
        "NCAAF": "42-27",
        "Soccer": "2-1",
    },
    "home_team_score": {
        "NBA": "112",
        "NFL": "31",
        "MLB": "5",
        "NHL": "4",
        "NCAAM": "78",
        "NCAAF": "42",
        "Soccer": "2",
    },
    "away_team_score": {
        "NBA": "105",
        "NFL": "24",
        "MLB": "3",
        "NHL": "2",
        "NCAAM": "65",
        "NCAAF": "27",
        "Soccer": "1",
    },
    # Winner/Loser
    "winner": {
        "NBA": "Detroit Pistons",
        "NFL": "Detroit Lions",
        "MLB": "Detroit Tigers",
        "NHL": "Detroit Red Wings",
        "NCAAM": "Michigan Wolverines",
        "NCAAF": "Michigan Wolverines",
        "Soccer": "Liverpool",
    },
    "winner_abbrev": {
        "NBA": "DET",
        "NFL": "DET",
        "MLB": "DET",
        "NHL": "DET",
        "NCAAM": "MICH",
        "NCAAF": "MICH",
        "Soccer": "LIV",
    },
    "loser": {
        "NBA": "Chicago Bulls",
        "NFL": "Chicago Bears",
        "MLB": "Chicago White Sox",
        "NHL": "Chicago Blackhawks",
        "NCAAM": "Ohio State Buckeyes",
        "NCAAF": "Ohio State Buckeyes",
        "Soccer": "Manchester City",
    },
    "loser_abbrev": {
        "NBA": "CHI",
        "NFL": "CHI",
        "MLB": "CWS",
        "NHL": "CHI",
        "NCAAM": "OSU",
        "NCAAF": "OSU",
        "Soccer": "MCI",
    },
    # Result variants
    "result": {
        "NBA": "W",
        "NFL": "W",
        "MLB": "W",
        "NHL": "W",
        "NCAAM": "W",
        "NCAAF": "W",
        "Soccer": "W",
    },
    "result_lower": {
        "NBA": "w",
        "NFL": "w",
        "MLB": "w",
        "NHL": "w",
        "NCAAM": "w",
        "NCAAF": "w",
        "Soccer": "w",
    },
    # Home/Away text
    "home_away_text": {
        "NBA": "Home",
        "NFL": "Home",
        "MLB": "Home",
        "NHL": "Home",
        "NCAAM": "Home",
        "NCAAF": "Home",
        "Soccer": "Home",
    },
    "is_away": {
        "NBA": "false",
        "NFL": "false",
        "MLB": "false",
        "NHL": "false",
        "NCAAM": "false",
        "NCAAF": "false",
        "Soccer": "false",
    },
    # Date variants
    "game_date_short": {
        "NBA": "Dec 22",
        "NFL": "Dec 22",
        "MLB": "Jul 12",
        "NHL": "Dec 21",
        "NCAAM": "Jan 11",
        "NCAAF": "Nov 30",
        "Soccer": "Dec 21",
    },
    "game_day": {
        "NBA": "Sunday",
        "NFL": "Sunday",
        "MLB": "Saturday",
        "NHL": "Saturday",
        "NCAAM": "Saturday",
        "NCAAF": "Saturday",
        "Soccer": "Saturday",
    },
    "game_day_short": {
        "NBA": "Sun",
        "NFL": "Sun",
        "MLB": "Sat",
        "NHL": "Sat",
        "NCAAM": "Sat",
        "NCAAF": "Sat",
        "Soccer": "Sat",
    },
    # Time variants
    "game_time.last": {
        "NBA": "7:30 PM EST",
        "NFL": "1:00 PM EST",
        "MLB": "7:10 PM EDT",
        "NHL": "7:00 PM EST",
        "NCAAM": "4:00 PM EST",
        "NCAAF": "12:00 PM EST",
        "Soccer": "10:00 AM EST",
    },
    # Venue .last variants
    "venue.last": {
        "NBA": "Rocket Mortgage FieldHouse",
        "NFL": "U.S. Bank Stadium",
        "MLB": "Target Field",
        "NHL": "Xcel Energy Center",
        "NCAAM": "Breslin Center",
        "NCAAF": "Spartan Stadium",
        "Soccer": "Stamford Bridge",
    },
    "venue_city.last": {
        "NBA": "Cleveland",
        "NFL": "Minneapolis",
        "MLB": "Minneapolis",
        "NHL": "St. Paul",
        "NCAAM": "East Lansing",
        "NCAAF": "East Lansing",
        "Soccer": "London",
    },
    "venue_full.last": {
        "NBA": "Rocket Mortgage FieldHouse, Cleveland, OH",
        "NFL": "U.S. Bank Stadium, Minneapolis, MN",
        "MLB": "Target Field, Minneapolis, MN",
        "NHL": "Xcel Energy Center, St. Paul, MN",
        "NCAAM": "Breslin Center, East Lansing, MI",
        "NCAAF": "Spartan Stadium, East Lansing, MI",
        "Soccer": "Stamford Bridge, London, England",
    },
    # Matchup .last/.next variants
    "matchup.last": {
        "NBA": "Detroit Pistons vs Cleveland Cavaliers",
        "NFL": "Detroit Lions vs Minnesota Vikings",
        "MLB": "Detroit Tigers vs Minnesota Twins",
        "NHL": "Detroit Red Wings vs Minnesota Wild",
        "NCAAM": "Michigan Wolverines vs Michigan State Spartans",
        "NCAAF": "Michigan Wolverines vs Michigan State Spartans",
        "Soccer": "Liverpool vs Chelsea",
    },
    "matchup.next": {
        "NBA": "Detroit Pistons vs Milwaukee Bucks",
        "NFL": "Detroit Lions vs Green Bay Packers",
        "MLB": "Detroit Tigers vs Cleveland Guardians",
        "NHL": "Detroit Red Wings vs Columbus Blue Jackets",
        "NCAAM": "Michigan Wolverines vs Indiana Hoosiers",
        "NCAAF": "Michigan Wolverines vs Penn State Nittany Lions",
        "Soccer": "Liverpool vs Arsenal",
    },
    # vs_at .last variant (uses "at" text)
    "vs_at.last": {
        "NBA": "at",
        "NFL": "at",
        "MLB": "at",
        "NHL": "at",
        "NCAAM": "at",
        "NCAAF": "vs",
        "Soccer": "vs",
    },
    # vs_@ variants (using @ symbol instead of 'at')
    "vs_@": {
        "NBA": "vs",
        "NFL": "vs",
        "MLB": "vs",
        "NHL": "vs",
        "NCAAM": "vs",
        "NCAAF": "vs",
        "Soccer": "vs",
    },
    "vs_@.next": {
        "NBA": "@",
        "NFL": "vs",
        "MLB": "@",
        "NHL": "@",
        "NCAAM": "@",
        "NCAAF": "vs",
        "Soccer": "@",
    },
    "vs_@.last": {
        "NBA": "@",
        "NFL": "@",
        "MLB": "@",
        "NHL": "@",
        "NCAAM": "@",
        "NCAAF": "vs",
        "Soccer": "vs",
    },
}


def get_sample_value(var_name: str, sport: str) -> str:
    """Get sample value for a variable and sport.

    Falls back through: exact match -> base var -> default
    """
    # Try exact match first
    if var_name in SAMPLE_DATA:
        sport_data = SAMPLE_DATA[var_name]
        if sport in sport_data:
            return sport_data[sport]
        # Fall back to first available sport
        return next(iter(sport_data.values()), "")

    # Try base variable (without .next/.last suffix)
    base_var = var_name.replace(".next", "").replace(".last", "")
    if base_var in SAMPLE_DATA:
        sport_data = SAMPLE_DATA[base_var]
        if sport in sport_data:
            return sport_data[sport]
        return next(iter(sport_data.values()), "")

    return ""


def get_all_sample_data(sport: str) -> dict[str, str]:
    """Get all sample values for a given sport."""
    result = {}
    for var_name, sport_data in SAMPLE_DATA.items():
        if sport in sport_data:
            result[var_name] = sport_data[sport]
        elif sport_data:
            # Fall back to first available
            result[var_name] = next(iter(sport_data.values()))
    return result
